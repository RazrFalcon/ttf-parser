...
impl<'a> Font<'a> {
    pub(crate) fn glyph_range(&self, glyph_id: u16) -> Result<Range32, Error> {
        ...
        let expected_len = match format {
            Format::Short => (self.number_of_glyphs as u32 + 1) * U16_LEN,
            Format::Long  => (self.number_of_glyphs as u32 + 1) * U32_LEN,
        };
        ...
        let (start, end) = match format {
            Format::Short => {
                s.skip((glyph_id as u32 * U16_LEN) as usize);
                // 'The actual local offset divided by 2 is stored.'
                (s.read_u16() as u32 * 2, s.read_u16() as u32 * 2)
            }
            Format::Long  => {
                s.skip((glyph_id as u32 * U32_LEN) as usize);
